...
<tr>
                                                        ...
                                                        <td>{{ $tables->parent }}</td>
                                                         <td>{{ $tables->phone }}</td>
                                                        <td>{{ $tables->email }}</td>
                                                        {{-- <td><a href="{{ route('staff.view',[$tables->_id]) }}" class="btn btn-primary">View</a></td> --}}

                                                        <td><a href="{{ route('staff.view',[$tables->_id]) }}" class="btn btn-primary">view</a></td>

                                                        <td>@if ($tables->isActive == 1)
                                                           <a href="{{ route('staff.deactive',[$tables->_id]) }}" class="btn btn-danger">Deactivate</a>
                                                           @else
                                                           <a href="{{ route('staff.active',[$tables->_id]) }}" class="btn btn-success">Activate</a>
                                                        @endif</td>
                                                        <td><a href="{{ route('staff.edit',[$tables->_id]) }}" class="btn btn-primary">Edit</a></td>
                                                        <td><a href="{{ route('staff.delete',[$tables->_id]) }}" class="btn btn-primary">Delete</a></td>
</tr>
                                                          @endforeach
