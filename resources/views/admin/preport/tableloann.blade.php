...
<tr>
                                                        ...
                                                        <td>@if ($tables->status == 1)
                                                           <a href="{{ route('preport.initateloann',[$tables->_id]) }}" class="btn btn-warning">Initated</a>
                                                           @elseif ($tables->status == 2)
                                                           <a href="{{ route('preport.processloann',[$tables->_id]) }}" class="btn btn-warning">Processing</a>
                                                            @elseif ($tables->status == 3)
                                                           <a href="{{ route('preport.approveloann',[$tables->_id]) }}" class="btn btn-success">Approved</a>
                                                           @elseif ($tables->status == 4)
                                                           <a href="{{ route('preport.rejectloann',[$tables->_id]) }}" class="btn btn-danger">Rejected</a>

                                                        @endif</td>
                                                      <td><a href="{{ route('preport.viewloann',[$tables->_id]) }}" class="btn btn-primary">View</a></td>
