...
<tr>
                                                        ...
                                                        <td>{{ $tables->categoryName }}</td>
                                                        <td>{{ $tables->icon }}</td>
                                                        {{-- <td><img src="data:image/png;base64, {{ $tables->image }}" width='10%' alt="Image Preview" /></td> --}}
                                                        <td>@if ($tables->is_Active == 1)
                                                           <a href="{{ route('category.deactive',[$tables->_id]) }}" class="btn btn-danger">Deactivate</a>
                                                           @else
                                                           <a href="{{ route('category.active',[$tables->_id]) }}" class="btn btn-success">Activate</a>
                                                        @endif</td>
                                                        {{-- <td><a href="{{ route('category.edit',[$tables->_id]) }}" class="btn btn-primary">Edit</a></td> --}}
                                                        <td><a href="{{ route('category.delete',[$tables->_id]) }}" class="btn btn-primary">Delete</a></td>
</tr>
                                                          @endforeach
