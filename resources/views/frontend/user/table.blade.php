...
<tr>
                                                        ...
                                                        <td>{{ $tables->name }}</td>
                                                        <td>{{ $tables->advisor }}</td>
                                                        <td>{{ $tables->phone }}</td>
                                                        <td>{{ $tables->phone1 }}</td>
                                                         <td>{{ $tables->phone2 }}</td>
                                                        <td>{{ $tables->email }}</td>
                                                        <td>{{ $tables->agencey }}</td>
                                                        <td>{{ $tables->licBranch }}</td>
                                                         <td>{{ $tables->city }}</td>
                                                        <td>{{ $tables->licOffice }}</td>
                                                        <td>{{ $tables->officer }}</td>
                                                        <td>{{ $tables->profile }}</td>
                                                        <td>{{ $tables->expiry }}</td>
                                                        <td>{{ $tables->date }}</td>
                                                        {{-- <td><img src="data:image/png;base64, {{ $tables->image }}" width='10%' alt="Image Preview" /></td> --}}
                                                        <td>@if ($tables->is_Active == 1)
                                                           <a href="{{ route('user.deactive',[$tables->_id]) }}" class="btn btn-danger">Deactivate</a>
                                                           @else
                                                           <a href="{{ route('user.active',[$tables->_id]) }}" class="btn btn-success">Activate</a>
                                                        @endif</td>
                                                        <td>@if ($tables->demo == 1)
                                                           <a href="{{ route('user.demo',[$tables->_id]) }}" class="btn btn-danger">Demo</a>
                                                           @else
                                                           <a href="{{ route('user.paid',[$tables->_id]) }}" class="btn btn-success">Paid</a>
                                                        @endif</td>
                                                        {{-- <td><a href="{{ route('user.edit',[$tables->_id]) }}" class="btn btn-primary">Edit</a></td> --}}
                                                        {{-- <td><a href="{{ route('user.delete',[$tables->_id]) }}" class="btn btn-primary">Delete</a></td> --}}
</tr>
                                                          @endforeach
